...
namespace HREngine.Bots
{
    ...
    /*
     ...
     For "omc" and "emc" you can use these extensions: 
        ...
        :Daragons (the number own/enemy Daragons on the board)
    ...
    public class RulesEngine
    {
        ...
        public enum param
        {
            ...
            emc_pirates_less,
            omc_daragons_equal,
            omc_daragons_notequal,
            omc_daragons_greater,
            omc_daragons_less,
            emc_daragons_equal,
            emc_daragons_notequal,
            emc_daragons_greater,
            emc_daragons_less,
            omc_shr_equal,
            omc_shr_notequal,
            omc_shr_greater,
        ...
        public void readRules(string behavName)
        {
        ...
        private bool validateCondition(string singlecond, string ruleString)
        {
            ...
            switch (parameter)
            {
                ...
                case "omc:daragons=": condParam = param.omc_daragons_equal; break;
                case "omc:daragons!=": condParam = param.omc_daragons_notequal; break;
                case "omc:daragons>": condParam = param.omc_daragons_greater; break;
                case "omc:daragons<": condParam = param.omc_daragons_less; break;
                case "emc:daragons=": condParam = param.emc_daragons_equal; break;
                case "emc:daragons!=": condParam = param.emc_daragons_notequal; break;
                case "emc:daragons>": condParam = param.emc_daragons_greater; break;
                case "emc:daragons<": condParam = param.emc_daragons_less; break;
                ...
                case "occ=": condParam = param.occ_equal; break; 
                case "occ!=": condParam = param.occ_notequal; break;
                case "occ>": condParam = param.occ_greater; break;
                case "occ<": condParam = param.occ_less; break;
                case "ecc=": condParam = param.ecc_equal; break; 
                case "ecc!=": condParam = param.ecc_notequal; break;
                case "ecc>": condParam = param.ecc_greater; break;
                case "ecc<": condParam = param.ecc_less; break;
        ...
        private bool checkCondition(Condition cond, Playfield p, Action a = null)
        {
            ...
            switch (cond.parameter)
            {
                ...
                case param.owa_equal: 
                    if (p.ownWeaponAttack == cond.num) return true;
                    return false;
                ...
                case param.ewa_equal: 
                    if (p.enemyWeaponAttack == cond.num) return true;
                    return false;
                ...
                case param.owd_equal: 
                    if (p.ownWeaponDurability == cond.num) return true;
                    return false;
                ...
                case param.ewd_equal: 
                    if (p.enemyWeaponDurability == cond.num) return true;
                    return false;
                ...
                case param.omc_less_emc:
                    if (p.ownMinions.Count < p.enemyMinions.Count) return true;
                    return false;
                case param.occ_equal: 
                    if (p.owncards.Count == cond.num) return true;
                    return false;
                case param.occ_notequal:
                    if (p.owncards.Count != cond.num) return true;
                    return false;
                case param.occ_greater:
                    if (p.owncards.Count > cond.num) return true;
                    return false;
                case param.occ_less:
                    if (p.owncards.Count < cond.num) return true;
                    return false;
                case param.ecc_equal: 
                    if (p.enemyAnzCards == cond.num) return true;
                    return false;
                case param.ecc_notequal:
                    if (p.enemyAnzCards != cond.num) return true;
                    return false;
                case param.ecc_greater:
                    if (p.enemyAnzCards > cond.num) return true;
                    return false;
                case param.ecc_less:
                    if (p.enemyAnzCards < cond.num) return true;
                ...
                case param.emc_pirates_less:
                    ...
                    if (tmp_counter < cond.num) return true;
                    return false;
                case param.omc_daragons_equal:
                    tmp_counter = 0;
                    foreach (Minion m in p.ownMinions) if ((TAG_RACE)m.handcard.card.race == TAG_RACE.DRAGON) tmp_counter++;
                    if (tmp_counter == cond.num) return true;
                    return false;
                case param.omc_daragons_notequal:
                    tmp_counter = 0;
                    foreach (Minion m in p.ownMinions) if ((TAG_RACE)m.handcard.card.race == TAG_RACE.DRAGON) tmp_counter++;
                    if (tmp_counter != cond.num) return true;
                    return false;
                case param.omc_daragons_greater:
                    tmp_counter = 0;
                    foreach (Minion m in p.ownMinions) if ((TAG_RACE)m.handcard.card.race == TAG_RACE.DRAGON) tmp_counter++;
                    if (tmp_counter > cond.num) return true;
                    return false;
                case param.omc_daragons_less:
                    tmp_counter = 0;
                    foreach (Minion m in p.ownMinions) if ((TAG_RACE)m.handcard.card.race == TAG_RACE.DRAGON) tmp_counter++;
                    if (tmp_counter < cond.num) return true;
                    return false;
                case param.emc_daragons_equal:
                    tmp_counter = 0;
                    foreach (Minion m in p.enemyMinions) if ((TAG_RACE)m.handcard.card.race == TAG_RACE.DRAGON) tmp_counter++;
                    if (tmp_counter == cond.num) return true;
                    return false;
                case param.emc_daragons_notequal:
                    tmp_counter = 0;
                    foreach (Minion m in p.enemyMinions) if ((TAG_RACE)m.handcard.card.race == TAG_RACE.DRAGON) tmp_counter++;
                    if (tmp_counter != cond.num) return true;
                    return false;
                case param.emc_daragons_greater:
                    tmp_counter = 0;
                    foreach (Minion m in p.enemyMinions) if ((TAG_RACE)m.handcard.card.race == TAG_RACE.DRAGON) tmp_counter++;
                    if (tmp_counter > cond.num) return true;
                    return false;
                case param.emc_daragons_less:
                    tmp_counter = 0;
                    foreach (Minion m in p.enemyMinions) if ((TAG_RACE)m.handcard.card.race == TAG_RACE.DRAGON) tmp_counter++;
                    if (tmp_counter < cond.num) return true;
                    return false;
                ...
                case param.ownweapon_equal: 
                    if (p.ownWeaponCard.cardIDenum == cond.cardID) return true;
                    return false;
                case param.ownweapon_notequal:
                    if (p.ownWeaponCard.cardIDenum != cond.cardID) return true;
                    return false;
                case param.enweapon_equal: 
                    if (p.enemyWeaponCard.cardIDenum == cond.cardID) return true;
                    return false;
