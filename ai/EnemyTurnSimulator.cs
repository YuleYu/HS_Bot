                ...
                switch (m.name)
                {
                    ...
                    case CardDB.cardName.shadowboxer:
                        ...
                        if (anz > 0)
                        {
                            ...
                            for (; anz > 0; anz--)
                            {
                                if (p.ownMinions.Count > 0) target = p.searchRandomMinion(p.ownMinions, Playfield.searchmode.searchLowestHP);
                    ...
                    case CardDB.cardName.unboundelemental:
                        ...
                        //****************************************
                    //****************************************turn
                  /*  case CardDB.cardName.gruul:
                        p.minionGetBuffed(m, 1, 1);
                        continue;
                    case CardDB.cardName.micromachine:
                        p.minionGetBuffed(m, 1, 0);
                        continue;*/
                    //****************************************armor
                    ...
                    /*case CardDB.cardName.cultmaster:
                        if (p.ownMinions.Count >=2 && p.enemyMinions.Count >= 3) p.drawACard(CardDB.cardName.unknown, false);
                        continue;
                    case CardDB.cardName.siltfinspiritwalker:
                        if (p.ownMinions.Count >=2 && p.enemyMinions.Count >= 3)
                        {
                            anz = 0;
                            foreach (Minion mnn in p.enemyMinions)
                            {
                                if ((TAG_RACE)mnn.handcard.card.race == TAG_RACE.MURLOC && mnn.entitiyID != m.entitiyID) anz++;
                            }
                            anz = Math.Min(anz, 3);
                            for (int i = 0; i < anz; i++)
                            {
                        ...
                        continue;
                    case CardDB.cardName.scavenginghyena:
                        if (p.ownMinions.Count >=2 && p.enemyMinions.Count >= 3)
                        {
                            anz = 0;
                            foreach (Minion mnn in p.enemyMinions)
                            {
                                if ((TAG_RACE)mnn.handcard.card.race == TAG_RACE.PET && mnn.entitiyID != m.entitiyID) anz++;
                            }
                            anz = Math.Min(anz, 2);
                            if (anz > 0) p.minionGetBuffed(m, anz * 2, anz * 1);
                        }
                        continue;
                    case CardDB.cardName.junkbot:
                        if (p.ownMinions.Count >=2 && p.enemyMinions.Count >= 3)
                        {
                            ...
                            foreach (Minion mnn in p.enemyMinions)
                            {
                                if ((TAG_RACE)mnn.handcard.card.race == TAG_RACE.MECHANICAL && mnn.entitiyID != m.entitiyID) anz++;
                            }
                            anz = Math.Min(anz, 2);
                            if (anz > 0) p.minionGetBuffed(m, anz * 2, anz * 2);
                        }
                        continue;
                    case CardDB.cardName.flesheatingghoul:
                        if (p.ownMinions.Count >=2 && p.enemyMinions.Count >= 3) p.minionGetBuffed(m, 1, 0);
                        continue;
                    case CardDB.cardName.mekgineerthermaplugg:
                        if (p.ownMinions.Count >=2) p.callKid(CardDB.Instance.getCardDataFromID(CardDB.cardIDEnum.EX1_029), p.enemyMinions.Count, false);
                        continue;*/
                    //****************************************
                }
            }
