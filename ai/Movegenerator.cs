        ...
        public List<Action> getMoveList(Playfield p, bool usePenalityManager, bool useCutingTargets, bool own)
        {
            ...
            if (own)
            {
                ...
                foreach (Handmanager.Handcard hc in p.owncards)
                {
                    ...
                    for (int choice = 0 + 1 * isChoice; choice < 1 + 2 * isChoice; choice++)
                    {
                        ...
                        trgts = c.getTargetsForCard(p, isLethalCheck, true);
                        if (trgts.Count == 0) continue;

                        int cardplayPenality = 0;
                        int bestplace = p.getBestPlace(c.type == CardDB.cardtype.MOB ? c : hc.card, isLethalCheck);
                        foreach (Minion trgt in trgts)
                        {
                            if (usePenalityManager) cardplayPenality = pen.getPlayCardPenality(c, trgt, p, isLethalCheck);
          ...
          // attack with hero (weapon)
            if ((own && p.ownHero.Ready && p.ownHero.Angr >= 1) || (!own && p.enemyHero.Ready && p.enemyHero.Angr >= 1))
            {
                ...
                foreach (Minion trgt in trgts)
                {
                    if ((own ? p.ownWeaponCard.name : p.enemyWeaponCard.name) == CardDB.cardName.foolsbane && trgt.isHero) continue;
           ...
           // use own ability
            if (own)
            {
                if (p.ownAbilityReady && p.mana >= p.ownHeroAblility.card.getManaCost(p, p.ownHeroAblility.manacost)) // if ready and enough manna
                {
                    ...
                    trgts = p.ownHeroAblility.card.getTargetsForCard(p, isLethalCheck, true);
                    foreach (Minion trgt in trgts)
                    {
                        if (usePenalityManager) cardplayPenality = pen.getPlayCardPenality(p.ownHeroAblility.card, trgt, p, isLethalCheck);
                        if (cardplayPenality <= 499)
                        {
