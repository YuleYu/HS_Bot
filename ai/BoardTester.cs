namespace HREngine.Bots
{
    ...
    public class BoardTester
    {
        ...
        CardDB.Card ownWeaponCard = new CardDB.Card();
        ...
        CardDB.Card enemyWeaponCard = new CardDB.Card();
        ...
        public BoardTester(string data = "")
        {
            ...
            foreach (string sss in lines)
            {
                ...
                if (readstate == 1 && counter == 2) // own hero weapon
                {
                    ...
                    if (ownHeroWeaponDurability == 0)
                    {
                        ownWeaponCard = new CardDB.Card();
                    }
                    else
                    {
                        if (s.Split(' ').Length > 5) ownWeaponCard = CardDB.Instance.getCardDataFromID(CardDB.Instance.cardIdstringToEnum(s.Split(' ')[4]));
                        else ownWeaponCard = CardDB.Instance.getCardData(CardDB.Instance.cardNamestringToEnum(s.Split(' ')[3]));
                    }
                }
                ...
                if (readstate == 2 && counter == 2) // wepon + stuff
                {
                    ...
                    {
                        if (s.Split(' ').Length > 5) enemyWeaponCard = CardDB.Instance.getCardDataFromID(CardDB.Instance.cardIdstringToEnum(s.Split(' ')[4]));
                        else enemyWeaponCard = CardDB.Instance.getCardData(CardDB.Instance.cardNamestringToEnum(s.Split(' ')[3]));
                    }
                }
            ...
            if (this.ownWeaponCard.windfury) herowindfury = true;
            ...
            //save data
            Hrtprozis.Instance.updateOwnHero(this.ownWeaponCard, this.ownHeroWeaponAttack, this.ownHeroWeaponDurability, this.ownheroname, heroability, abilityReady, this.ownHeroPowerCost, this.ownHero);
            Hrtprozis.Instance.updateEnemyHero(this.enemyWeaponCard, this.enemyWeaponAttack, this.enemyWeaponDur, this.enemyheroname, enemmaxman, enemyability, this.enemyHeroPowerCost, this.enemyHero);
